...
class DetectionManager:
    ...
    def __init__(self, db_path: str = "vision_cache.db"):
        ...
        self.vision_api = VisionAPIClient()
    ...
    def _find_similar_cached_response(self, embedding: np.ndarray, prompt_type: str, 
        ...
        try:
            ...
            for embedding_hash, api_response, hit_count in cached_entries:
                # Parse cached embedding from hash
                try:
            ...
            if best_response:
                ...
                # Update hit count
                self._update_hit_count(embedding_hash)
    ...
    def _should_trigger_api_call(self, object_class: str, confidence: float) -> bool:
        ...
        if confidence < self.confidence_threshold:
            logger.debug(f"Confidence {confidence:.2f} below threshold {self.confidence_threshold}")
            return False
...
def test_detection_manager():
    """Test the DetectionManager functionality"""
    print("=== DetectionManager Test ===")
    ...
    cv2.rectangle(test_frame, (100, 100), (200, 200), (0, 255, 0), -1)  # Green rectangle
    cv2.putText(test_frame, "BANANA", (110, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
    
    cv2.rectangle(test_frame, (300, 200), (400, 300), (255, 0, 0), -1)  # Red rectangle
    cv2.putText(test_frame, "APPLE", (320, 250), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    
    print("Processing test frame...")
    results = manager.process_frame(test_frame)
    
    print(f"Detections: {results['stats']['total_detections']}")
    print(f"API calls made: {results['stats']['api_calls_made']}")
    print(f"Cache hits: {results['stats']['cache_hits']}")
    print(f"Cooldown skipped: {results['stats']['cooldown_skipped']}")
    
    # Print API responses
    for response in results['api_responses']:
        print(f"API Response for {response['class']}: {response['response'][:100]}...")
        
    # Print cached responses
    for response in results['cached_responses']:
        print(f"Cached Response for {response['class']}: {response['response'][:100]}...")
